...
class HaciendaApi:
    ...
    def recepcion_dte(self, doc):
        ...
        if not isinstance(kernel, dict):
            raise UserError("Error en la autenticacion: \n %s" % kernel)
            # return False
    ...
    def contingency_dte(self, doc):
        ...
        env = self.env
        headers = {'Authorization': kernel.get('token', ''), 'Content-type': 'application/json'}
        data = {'nit': self.company_id.l10n_sv_mh_auth_user,
                'documento': doc.json_codte_signed,
                }
        _logger.info(data)
        environment = HACIENDA_ENV_MAP[env]
        endpoint = "https://{}.dtes.mh.gob.sv/fesv/contingencia".format(environment)
        response = requests.post(endpoint, json=data, headers=headers)
        _logger.info(response.status_code)
        _logger.info(response.text)
        return response
