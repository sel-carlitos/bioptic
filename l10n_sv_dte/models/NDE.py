...
class Receptor(GeneratedsSuper):
    ...
    def __init__(self, nit=None, nombre=None, tipoDocumento=None, numDocumento=None, nrc=None, codActividad=None,
                 descActividad=None, direccion=None, telefono=None, correo=None, nombreComercial=None):
        self.original_tagname_ = None
        self.nit = nit
        self.nombre = nombre
        self.tipoDocumento = tipoDocumento
...
class Resumen(GeneratedsSuper):
    ...
    def __init__(self, totalNoSuj=0.00, totalLetras=None, ivaRete1=0.00,
                 subTotalVentas=0.00, subTotal=0.00, reteRenta=0.00, descuExenta=0.00,
                 totalDescu=0.00, tributos=None, descuGravada=0.00, totalGravada=0.00,
                 montoTotalOperacion=0.00, ivaPerci1=0.00,
                 totalExenta=0.00, descuNoSuj=0.00, condicionOperacion=None, numPagoElectronico=None):
        self.original_tagname_ = None
    ...
    def hasContent_(self):
        if (
                ...
                self.numPagoElectronico is not None
        ):
            return True
        else:
            return False
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='EmisorType', fromsubclass_=False,
        ...
        if self.ivaPerci1 is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write(bytes(('"ivaPerci1":%s,%s' % (
                self.gds_encode(
                    self.gds_format_float(self.ivaPerci1, input_name='Sub-Total')),
                eol_)).encode()))
