...
class SVTerminal(models.Model):
    ...
    def action_confirm(self):
        for rec in self:
            ...
            vals = {'code': rec.code}
            rec.sequence_fe = self._create_sequence(name="Factura electrónica", code="01", pref="fe",
                                                    vals=vals, location=location).id
            rec.sequence_ccfe = self._create_sequence(name="Comprobante de Crédito Fiscal Electrónico", code="03",
                                                      pref="ccfe", vals=vals, location=location).id
            rec.sequence_nre = self._create_sequence(name="Nota de Remisión Electrónica", code="04",
                                                     pref="nre", vals=vals, location=location).id
            rec.sequence_nce = self._create_sequence(name="Nota de Crédito Electrónica", code="05",
                                                     pref="nce", vals=vals, location=location).id
            rec.sequence_nde = self._create_sequence(name="Nota de Débito Electrónica", code="06",
                                                     pref="nde", vals=vals, location=location).id
            rec.sequence_cre = self._create_sequence(name="Comprobante de Retencion Electronico", code="07",
                                                     pref="cre", vals=vals, location=location).id
            rec.sequence_fex = self._create_sequence(name="Factura electrónica de exportación", code="11",
                                                     pref="fee", vals=vals, location=location).id
            rec.sequence_fsee = self._create_sequence(name="Factura Sujeto Excluido Electrónico", code="14",
                                                      pref="fsee", vals=vals, location=location).id
            rec.sequence_cde = self._create_sequence(name="Comprobante de Donación Electrónica", code="15",
                                                     pref="cde", vals=vals, location=location).id
            rec.state = 'active'
    ...
    def action_view_sequence(self):
        ...
        domain = [self.sequence_fe.id,  self.sequence_nre.id, self.sequence_nde.id, self.sequence_nce.id, self.sequence_ccfe.id,
                  self.sequence_fex.id, self.sequence_fsee.id, self.sequence_cde.id, self.sequence_cre.id]
        if domain:
